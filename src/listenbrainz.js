...
const listensURL = `${listenBrainz}/user/${user}/listens?count=3`;
const playingURL = `${listenBrainz}/user/${user}/playing-now`;
const root = "/src/assets/images/album-covers/";
...
function newAlbum(artist, album, brainzID) {
    ...
    favAlbums += `<div class="album" albm="${album}">
                        <a rel="noopener" title="${album}" href="https://listenbrainz.org/album/${brainzID}" target="_blank"> 
                            <img title="${artist} - ${album}"src="${root}${rep(artist)}-${rep(album)}.webp" alt="" class="albumCover">
                        </a>
                    </div>`;
}
function fetchListens() {
    ...
    // now listening fetch
    fetchJsonRetry(playingURL, 2).then((playingData) => {
        if (playingData.payload.listens.length > 0) {
            console.log("listenbrainz is WOKE LEFT!!!");
            const playingDataPath = playingData.payload.listens[0].track_metadata;
            nowPlayingHTML.innerHTML = 
            `<nobr>now playing! <b class="musicHover periwinkle" title="${playingDataPath.track_name}"><br>${playingDataPath.track_name}</b><br>
            by <b class="musicHover periwinkle" title="${playingDataPath.artist_name}"><br>${playingDataPath.artist_name}</b><br>
            from <b class="musicHover periwinkle" title="${playingDataPath.release_name}"><br>${playingDataPath.release_name}</b></b></nobr>`;
            nowPlayingCover.setAttribute('src', `https://coverartarchive.org/release/${playingDataPath.additional_info.release_mbid}/front-250.jpg`);
            nowPlayingCover.setAttribute('title', `${playingDataPath.artist_name} - ${playingDataPath.release_name} on listenbrainz!!!`);
            nowPlayingCover.setAttribute('alt', `${playingDataPath.artist_name} - ${playingDataPath.release_name}`);
            coverLink.setAttribute('href', `https://listenbrainz.org/album/${playingDataPath.additional_info.release_group_mbid}`);
        }
        else {
    ...
    // recent listen fetch :3
           // recent listen fetch :3
    fetchJsonRetry(listensURL, 2).then((listensData) => {
        function feedListen(e, value) {
            const dataPath = listensData.payload.listens[value].track_metadata;
            const listenTime = new Date(listensData.payload.listens[value].listened_at * 1000);
            const day = listenTime.getDate()
            const month = listenTime.getMonth()
            const hours = listenTime.getHours();
            const minutes = `0${listenTime.getMinutes()}`;
            const time = `${hours}:${minutes.substr(-2)} on ${day}/${month}`

            e.innerHTML = `<nobr>listened to at <b class="grey"">${time}</b><b class="musicHover periwinkle" title="${dataPath.track_name}"><br>${dataPath.track_name}</b><br>
